// services/ontologyService.js - Fixed version with proper SPARQL syntax
...
async function fetchOntologyMetadata(uri) {
  try {
    ...
    // Use properly formatted SPARQL query with the URI in angle brackets
    const query = `
...
async function fetchProductsForOntology(uri) {
  try {
    ...
    // FIXED: Properly format SPARQL with string literals in FILTER clauses
    const query = `
...
async function fetchOntologyRelationships(uri) {
  try {
    ...
    // FIXED: Properly format the FILTER clause without nested angle brackets
    const query = `
      SELECT DISTINCT ?property ?propertyLabel ?domain ?domainLabel ?range ?rangeLabel WHERE {
        # Get object properties defined in this ontology
        {
          ...
          FILTER(STRSTARTS(STR(?property), "${safeUri}"))
...
async function generateDescriptionFromContent(uri) {
  ...
  // FIXED: Properly format the FILTER clause
  const classQuery = `
    SELECT ?class ?label
    WHERE {
      {
        ?class a <http://www.w3.org/2002/07/owl#Class> .
        FILTER(STRSTARTS(STR(?class), "${safeUri}"))
      } 
      UNION 
      {
...
async function getOntologyStats(uri) {
  try {
    ...
    // FIXED: Properly format the FILTER clauses
    const classesQuery = `
      ...
      SELECT (COUNT(DISTINCT ?individual) AS ?count) WHERE {
        {
          ...
          FILTER(STRSTARTS(STR(?individual), "${safeUri}"))
        }
        UNION
        {
