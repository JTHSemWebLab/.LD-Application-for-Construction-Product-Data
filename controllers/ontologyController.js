...
exports.downloadOntology = async (req, res, next) => {
  try {
    ...
    const mimeType = mimeTypes[format] || 'application/rdf+xml';
    
    // Create the export URL for GraphDB
    const exportUrl = `${graphdbConfig.endpoint}/repositories/${graphdbConfig.repository}/statements`;
    
    try {
      // Make request to GraphDB with authentication
      ...
      res.setHeader('Content-Disposition', `attachment; filename="${uri.split('/').pop()}.${format}"`);
      
      // Pipe the response directly to the client
      response.data.pipe(res);
    } catch (graphDbError) {
      console.error('Error downloading from GraphDB:', graphDbError);
      
      // If context-based export fails, try a different approach
      // Query for all triples about this ontology
      const query = `
        CONSTRUCT { ?s ?p ?o }
        WHERE {
          {
      ...
      res.setHeader('Content-Type', mimeType);
      res.setHeader('Content-Disposition', `attachment; filename="${uri.split('/').pop()}.${format}"`);
      res.send(constructResponse.data);
    }
  } catch (err) {
    console.error('Error in downloadOntology:', err);
    next(err);
  }
};
